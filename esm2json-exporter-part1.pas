...
begin
  Result := (Pos('CELL', path_string) = 0) And (Pos('WRLD', path_string) = 0);
end;
...
var
  ...
  sig, x_string, parent_path, parent_basename, path_string: string;
...
begin
  ...
  begin
    ...
    AddMessage('Pre-scan (non-CELL/WRLD) total records: ' + IntToStr(progress_total));
...
//  json_output := TStringList.Create;
  ...
  begin
    AddMessage('ERROR: Unable to create directory: ' + element_path);
    Exit;
  end;
